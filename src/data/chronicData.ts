...
/**
 * Returns the chronic patient list with all displayed strings (name, village,
 * worker, conditionLabel, alert messages, checkup types) localized to the
 * current UI language.  Components should use this instead of `chronicPatients`
 * directly.
 */
export function useLocalizedChronicPatients(): ChronicPatient[] {
  const { language } = useApp()
  const L = createLocalizer(language)

  return chronicPatients.map(p => ({
    ...p,
    name:           L.name(p.name),
    village:        L.village(p.village),
    worker:         L.name(p.worker),
    conditionLabel: L.condition(p.conditionLabel),
    alerts: p.alerts.map(a => ({
      ...a,
      // Keep the English alert message — it contains BP numbers / percentages
      // that are universally understood and should not be garbled by pattern match.
    })),
    checkups: p.checkups.map(c => ({
      ...c,
      // e.g. "BP check + medication review" — localise only the condition part
      type: L.condition(c.type) !== c.type ? L.condition(c.type) : c.type,
    })),
  }))
}
