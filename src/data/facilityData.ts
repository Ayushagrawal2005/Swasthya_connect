...
/**
 * Returns the facilities array with name, doctor names and specialties all
 * localized to the current UI language.
 */
export function useLocalizedFacilities(): Facility[] {
  const { language } = useApp()
  const L = createLocalizer(language)

  return facilities.map(f => ({
    ...f,
    name: L.facility(f.name),
    doctors: f.doctors.map(d => ({
      ...d,
      name:      L.name(d.name),
      specialty: L.specialty(d.specialty),
    })),
  }))
}

/** Localized version of tierLabel map. */
export function useLocalizedTierLabel(): Record<FacilityTier, string> {
  const { language } = useApp()
  const L = createLocalizer(language)
  return {
    'sub-centre':     L.tier('sub-centre'),
    phc:              L.tier('phc'),
    'rural-hospital': L.tier('rural-hospital'),
    district:         L.tier('district'),
  }
}
