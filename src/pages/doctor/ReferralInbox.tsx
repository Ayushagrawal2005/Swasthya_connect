    ...
    <div className="p-4 sm:p-6 max-w-2xl mx-auto space-y-5 animate-fade-in">
        ...
        {(['incoming', 'outgoing'] as Tab[]).map(t2 => (
          <button key={t2} role="tab" aria-selected={tab === t2}
            ...
            className={`px-5 py-2.5 text-sm font-medium capitalize border-b-2 transition-colors
              ${tab === t2 ? 'border-teal-500 text-teal-600' : 'border-transparent text-[#5F5E5A] hover:text-[#2C2C2A]'}`}>
            {t2 === 'incoming' ? t('incoming') : t('outgoing')}
            <span className={`ml-2 text-[10px] px-1.5 py-0.5 rounded-full ${t2 === 'incoming' ? 'bg-coral-100 text-coral-700' : 'bg-gray-100 text-[#5F5E5A]'}`}>
      ...
      <div className="space-y-3" role="list">
        {loading ? (
          ...
          <p className="text-sm text-[#5F5E5A] text-center py-8">{t('noReferrals')}</p>
        ) : list.map((ref, i) => {
          return (
            <motion.article key={ref.id} role="listitem"
              ...
              <div className="flex items-start justify-between gap-2 flex-wrap">
                <div>
                  <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                    <p className="font-semibold text-sm text-[#2C2C2A]">{L.name(ref.patientName)}</p>
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-[#5F5E5A]">
                    ...
                    {L.facility('Sub-Centre Mandav')} <ArrowRight size={11} /> {L.facility(ref.toFacilityName)}
                  </div>
                </div>
                <div className="flex gap-1.5 flex-wrap">
                  <span className={`${urgencyBadge[ref.urgency]} text-[10px] capitalize`}>{L.status(ref.urgency)}</span>
                  <span className={`${statusBadge[ref.status] || 'badge-amber'} text-[10px] capitalize`}>{L.status(ref.status)}</span>
                </div>
              </div>
              ...
              {tab === 'incoming' && ref.status === 'pending' && (
                <div className="flex gap-2 pt-1">
                  <button onClick={() => accept(ref.id)}
                    ...
                    <CheckCircle size={13} /> {t('acceptReferral')}
                  </button>
                  <button onClick={() => redirect(ref.id)}
                    ...
                    <XCircle size={13} /> {t('redirectReferral')}
                  </button>
                </div>
              )}
              ...
              {ref.status === 'accepted' && (
                <div className="flex items-center gap-2 text-xs text-green-700 bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                  <CheckCircle size={13} /> {t('accepted')} — {t('patientQueueLabel')}
                </div>
              )}

              {ref.urgency === 'emergency' && (
                <div className="flex items-center gap-2 text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2">
                  <AlertTriangle size={13} /> {t('emergency')} — {t('urgent')}
                </div>
              )}
            </motion.article>
          )
