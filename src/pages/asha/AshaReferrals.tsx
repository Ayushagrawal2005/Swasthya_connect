  ...
  return (
    <div className="p-4 sm:p-6 max-w-2xl mx-auto space-y-5 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-[#2C2C2A]">{t('referrals')}</h1>
          <p className="text-sm text-[#5F5E5A] mt-0.5">{referrals.filter(r => r.status === 'pending').length} {t('pending')} · {L.facility('Sub-Centre Mandav')}</p>
        </div>
                  ...
                  <div className="flex items-start justify-between gap-2 flex-wrap">
                    <div>
                      ...
                      <div className="flex items-center gap-1.5 text-xs text-[#5F5E5A]">
                        <MapPin size={11} className="text-teal-500" /> {L.facility('Sub-Centre Mandav')}
                        <ArrowRight size={11} /> {L.facility(ref.toFacilityName)}
                      </div>
                    </div>
                    <div className="flex gap-1.5">
                      <span className={`${urgencyBadge[ref.urgency]} text-[10px] capitalize`}>{L.status(ref.urgency)}</span>
                      <span className={`${statusBadge[ref.status]} text-[10px]`}>{L.status(ref.status)}</span>
                    </div>
                  </div>
