...
async function seed() {
  ...
  try {
    ...
    const users = [
      ...
      { username: 'patient2', password: 'password', role: 'patient', name: 'Ramesh Kale', email: 'ramesh@example.com', phone: '9876543241' }
    ]
    ...
    // Link patient users to their patient records
    const patient1User = createdUsers.find(u => u.username === 'patient1')
    const patient2User = createdUsers.find(u => u.username === 'patient2')
    const meenaPatientRecord = createdPatients.find(p => p.name === 'Meena Jadhav')
    const rameshPatientRecord = createdPatients.find(p => p.name === 'Ramesh Kale')
    
    if (patient1User && meenaPatientRecord) {
      await db.users.update(patient1User.id, { patientId: meenaPatientRecord.id })
      console.log(`   ✓ Linked patient1 user to Meena's patient record`)
    }
    
    if (patient2User && rameshPatientRecord) {
      await db.users.update(patient2User.id, { patientId: rameshPatientRecord.id })
      console.log(`   ✓ Linked patient2 user to Ramesh's patient record`)
    }
    ...
    // ═══════════════════════════════════════════════════════════════
    // SEED WELLNESS DATA (Family Members & Vaccinations)
    // ═══════════════════════════════════════════════════════════════
    console.log('\n🏥 Creating wellness data...')
    
    // Find Meena (female patient) and create her menstrual & pregnancy data
    const meenaPatient = createdPatients.find(p => p.name === 'Meena Jadhav')
    const rameshPatient = createdPatients.find(p => p.name === 'Ramesh Kale')
    
    // Add family members
    if (meenaPatient) {
      console.log('   Creating family members for Meena...')
      const child1 = await db.familyMembers.create({
        guardianPatientId: meenaPatient.id,
        name: 'Aarav Jadhav',
        relationship: 'child',
        gender: 'M',
        dob: '2021-05-15',
      ...
      const child2 = await db.familyMembers.create({
        guardianPatientId: meenaPatient.id,
        name: 'Ananya Jadhav',
        relationship: 'child',
        gender: 'F',
      ...
      console.log(`   ✓ Added 2 children for Meena`)
      
      // Add menstrual cycles for Meena
      const cycles = [
        { startDate: '2024-11-01', endDate: '2024-11-05', cycleLength: 5, flowIntensity: 'moderate', symptoms: ['Cramps', 'Fatigue'] },
        { startDate: '2024-11-29', endDate: '2024-12-03', cycleLength: 5, flowIntensity: 'moderate', symptoms: ['Headache'] },
        { startDate: '2024-12-27', endDate: '2024-12-31', cycleLength: 4, flowIntensity: 'light', symptoms: [] },
        { startDate: '2025-01-25', endDate: '2025-01-29', cycleLength: 4, flowIntensity: 'moderate', symptoms: ['Cramps'] }
      ]
      
      for (const cycle of cycles) {
        await db.menstrualCycles.create({
          patientId: meenaPatient.id,
          ...cycle,
          notes: ''
        })
      }
      console.log(`   ✓ Added ${cycles.length} menstrual cycles for Meena`)
      
      // Add vaccinations for Meena's children
      const today = new Date()
      const vaccinations = [
        // Aarav (3 years old) - completed vaccinations
        { familyMemberId: child1.id, vaccineName: 'BCG', scheduledDate: '2021-06-01', dueDate: '2021-06-15', status: 'completed', administeredDate: '2021-06-10' },
        { familyMemberId: child1.id, vaccineName: 'OPV-1', scheduledDate: '2021-07-01', dueDate: '2021-07-15', status: 'completed', administeredDate: '2021-07-12' },
        { familyMemberId: child1.id, vaccineName: 'DPT-1', scheduledDate: '2021-08-01', dueDate: '2021-08-15', status: 'completed', administeredDate: '2021-08-08' },
        { familyMemberId: child1.id, vaccineName: 'MMR-1', scheduledDate: '2022-05-01', dueDate: '2022-05-15', status: 'completed', administeredDate: '2022-05-10' },
        // Ananya (1 year old) - some pending
        { familyMemberId: child2.id, vaccineName: 'BCG', scheduledDate: '2023-09-01', dueDate: '2023-09-15', status: 'completed', administeredDate: '2023-09-05' },
        { familyMemberId: child2.id, vaccineName: 'OPV-1', scheduledDate: '2023-10-01', dueDate: '2023-10-15', status: 'completed', administeredDate: '2023-10-10' },
        { familyMemberId: child2.id, vaccineName: 'DPT-1', scheduledDate: '2024-11-01', dueDate: '2024-11-15', status: 'completed', administeredDate: '2024-11-08' },
        { familyMemberId: child2.id, vaccineName: 'MMR-1', scheduledDate: new Date(today.getTime() + 30*24*60*60*1000).toISOString().split('T')[0], dueDate: new Date(today.getTime() + 45*24*60*60*1000).toISOString().split('T')[0], status: 'upcoming', notes: 'Schedule at PHC' },
        // Meena's self vaccinations
        { patientId: meenaPatient.id, vaccineName: 'COVID-19 Booster', scheduledDate: '2024-01-15', dueDate: '2024-01-31', status: 'completed', administeredDate: '2024-01-20' },
        { patientId: meenaPatient.id, vaccineName: 'Tetanus Toxoid', scheduledDate: new Date(today.getTime() + 15*24*60*60*1000).toISOString().split('T')[0], dueDate: new Date(today.getTime() + 30*24*60*60*1000).toISOString().split('T')[0], status: 'due', notes: 'Annual booster' }
      ]
      
      for (const vax of vaccinations) {
        await db.vaccinations.create(vax)
      }
      console.log(`   ✓ Added ${vaccinations.length} vaccinations`)
    }
    
    // Add family member for Ramesh (male patient)
    if (rameshPatient) {
      console.log('   Creating family members for Ramesh...')
      const spouse = await db.familyMembers.create({
        guardianPatientId: rameshPatient.id,
        name: 'Savita Kale',
        relationship: 'spouse',
        gender: 'F',
        dob: '1968-03-10',
      ...
      console.log(`   ✓ Added spouse for Ramesh`)
      
      // Add Ramesh's self vaccinations
      const today = new Date()
      const rameshVax = [
        { patientId: rameshPatient.id, vaccineName: 'Influenza', scheduledDate: '2024-10-01', dueDate: '2024-10-15', status: 'completed', administeredDate: '2024-10-05' },
        { patientId: rameshPatient.id, vaccineName: 'COVID-19 Annual', scheduledDate: new Date(today.getTime() + 60*24*60*60*1000).toISOString().split('T')[0], dueDate: new Date(today.getTime() + 75*24*60*60*1000).toISOString().split('T')[0], status: 'upcoming' }
      ]
      
      for (const vax of rameshVax) {
        await db.vaccinations.create(vax)
      }
      console.log(`   ✓ Added ${rameshVax.length} vaccinations for Ramesh`)
    }
    
    console.log('\n🎉 You can now:')
    console.log('   1. Login with: asha1/password, doctor1/password, admin1/password, patient1/password')
    console.log('   2. Female patient: patient1/password (Meena - has menstrual cycles, 2 children)')
    console.log('   3. Male patient: patient2/password (Ramesh - has spouse, vaccinations)')
    console.log('   4. View data in Firebase Console: https://console.firebase.google.com/')
    console.log('   5. Start the backend: npm run dev')
    console.log('   6. Start the frontend: npm run dev\n')
