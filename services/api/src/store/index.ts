...
export interface PregnancyTracker {
  ...
  checkups: Array<{ date: string; notes: string; worker: string }>
...
export interface Notification {
  id: string
  userId: string
  type: 'followup-created' | 'followup-due' | 'followup-overdue' | 'followup-completed' | 'followup-escalated' | 'referral-update' | 'appointment-reminder' | 'system'
  title: string
  message: string
  read: boolean
  actionUrl?: string
  relatedId?: string
  relatedType?: 'followup' | 'referral' | 'appointment' | 'patient'
  createdAt: string
}
...
export interface FollowUp {
  ...
  sourcePortal: FollowUpSource
  sourceId?: string               // referralId, appointmentId, etc.
  reminderSchedule: string[]      // ISO dates when reminders should be sent
...
export interface MedicationEntryNew {
  id: string
  medicineName: string
  frequency: string
  dose: string
...
export interface TriageSessionExtended {
  id: string
  patientId: string
  visitId?: string
  initiatedBy: string
  ...
  medications: MedicationEntryNew[]
  documentIds: string[]
  summaryId?: string
  summaryText?: string
  riskLevel?: RiskLevel
...
export interface ConsultationNotes {
  ...
  diagnosis?: string
  notes: string
  prescription: MedicationEntryNew[]
  advice: string
...
export const db = {
  ...
  menstrualCycles: [] as MenstrualCycle[],
  pregnancyTrackers: [] as PregnancyTracker[],
  ...
  // Extended teleconsult collections
  triageSessionsExtended: [] as TriageSessionExtended[],
  triageSummaries: [] as TriageSummary[],
  teleconsultQueue: [] as TeleconsultQueue[],
  consultationNotes: [] as ConsultationNotes[],
  documentUploads: [] as DocumentUpload[],
  longitudinalRecords: [] as LongitudinalRecord[],
}
...
export function seedStore() {
  ...
  db.followUps.push(
    { id: uuid(), patientId: P1, patientName: 'Meena Patil', age: 32, phone: '9876543210', condition: 'Gestational Hypertension', risk: 'high', dueDate: today, status: 'due-today', notes: 'BP worsening trend — escort to PHC if possible', nextStep: 'PHC visit today', lastVisit: '23 Aug 2026', assignedTo: U_ASHA, assignedToName: 'ANM Kavita Shinde', createdBy: U_DOC, createdByName: 'Dr. Ramesh Patil', sourcePortal: 'doctor', reminderSchedule: [today], createdAt: now, updatedAt: now },
    { id: uuid(), patientId: P3, patientName: 'Ganesh Wagh', age: 48, phone: '9823456789', condition: 'TB (DOTS)', risk: 'emergency', dueDate: today, status: 'overdue', notes: 'CRITICAL: 60% compliance — drug resistance risk', nextStep: 'Home visit — DOTS observation', lastVisit: '16 Aug 2026', assignedTo: U_ASHA, assignedToName: 'ANM Kavita Shinde', createdBy: U_ASHA, createdByName: 'ANM Kavita Shinde', sourcePortal: 'asha', reminderSchedule: [today], createdAt: now, updatedAt: now },
    { id: uuid(), patientId: P4, patientName: 'Lata Desai', age: 62, phone: '9765432100', condition: 'Type 2 Diabetes', risk: 'medium', dueDate: '2026-08-25', status: 'upcoming', notes: 'HbA1c slight rise — dietary review', nextStep: 'Phone follow-up', lastVisit: '23 Aug 2026', assignedTo: U_ASHA, assignedToName: 'ANM Kavita Shinde', createdBy: U_DOC, createdByName: 'Dr. Ramesh Patil', sourcePortal: 'doctor', reminderSchedule: ['2026-08-24', '2026-08-25'], createdAt: now, updatedAt: now },
    { id: uuid(), patientId: P5, patientName: 'Suresh Kadam', age: 55, phone: '9812345678', condition: 'CKD Stage 2', risk: 'medium', dueDate: '2026-08-22', status: 'overdue', notes: 'Missed creatinine test', nextStep: 'Reschedule lab', lastVisit: '01 Aug 2026', assignedTo: U_ASHA, assignedToName: 'ANM Kavita Shinde', createdBy: U_ASHA, createdByName: 'ANM Kavita Shinde', sourcePortal: 'asha', reminderSchedule: ['2026-08-21', '2026-08-22'], createdAt: now, updatedAt: now },
    { id: uuid(), patientId: P2, patientName: 'Priya Sharma', age: 28, phone: '9765432109', condition: 'Anaemia', risk: 'low', dueDate: '2026-09-01', status: 'upcoming', notes: 'IFA compliance good. Routine check', nextStep: 'Monthly check', lastVisit: '10 Aug 2026', assignedTo: U_DOC, assignedToName: 'Dr. Ramesh Patil', createdBy: U_DOC, createdByName: 'Dr. Ramesh Patil', sourcePortal: 'doctor', reminderSchedule: ['2026-08-31'], createdAt: now, updatedAt: now },
  )
