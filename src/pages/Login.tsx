    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-stretch">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 sm:p-10 overflow-y-auto">
        ...
        >
          ...
          {/* Sign-in form */}
          <h2 className="text-2xl font-semibold text-[#2C2C2A] mb-1">{t('signIn')}</h2>
          <p className="text-[#5F5E5A] text-sm mb-6">Select a demo account above or enter your credentials.</p>

          <form onSubmit={handleSignIn} noValidate className="space-y-4">
            <div>
              <label htmlFor="username" className="block text-sm font-medium text-[#2C2C2A] mb-1.5">{t('username')}</label>
            ...
            <div>
              <label htmlFor="password" className="block text-sm font-medium text-[#2C2C2A] mb-1.5">{t('password')}</label>
            ...
            <button type="submit" disabled={loading || !username || !password}
              ...
              {loading ? (
                ...
                <>{t('signIn')} <ArrowRight size={17} aria-hidden="true" /></>
              )}
            </button>
          </form>
        </motion.div>
