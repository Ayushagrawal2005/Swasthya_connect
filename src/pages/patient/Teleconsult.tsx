// Module 3 - Teleconsultation + Adaptive bandwidth streamer
import { useState, useEffect } from "react"
import { motion, AnimatePresence } from "framer-motion"
import { Video, Mic, MicOff, VideoOff, Phone, MessageSquare, CheckCircle, Wifi, WifiOff, Monitor, Volume2 } from "lucide-react"
...
const preChecks = [
  { label: "Camera working",      ok: true },
  { label: "Microphone",          ok: true },
  { label: "Network connection",  ok: true },
  { label: "Speaker / earphones", ok: true },
]
...
export function TeleconsultPage() {
  ...
  useEffect(() => {
    ...
    const t1 = setTimeout(() => { setBand("poor"); setBandToast("Network degraded - SD video (3G)"); setTimeout(() => setBandToast(""), 3000) }, 8000)
    const t2 = setTimeout(() => { setBand("critical"); setBandToast("Weak signal - switched to audio-only (2G)"); setTimeout(() => setBandToast(""), 4000) }, 16000)
    const t3 = setTimeout(() => { setBand("good"); setBandToast("Network restored - video reconnected (4G)"); setTimeout(() => setBandToast(""), 3000) }, 24000)
  ...
  function sendMsg() {
    ...
    setChatMsg("")
    setTimeout(() => setMessages(p => [...p, { from: "doctor", text: "Understood. I will review your BP trend and adjust medication." }]), 1500)
  }
          ...
          <div className="card p-4 w-full">
            ...
            {preChecks.map(c => (
              <div key={c.label} className="flex items-center justify-between text-sm py-1">
                <span className="text-[#5F5E5A]">{c.label}</span>
                <CheckCircle size={16} className="text-green-500" />
              </div>
            ))}
          </div>
          <p className="text-xs text-center text-indigo-700 bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-2">
            Adaptive streaming enabled - if signal drops to 2G/3G the call auto-switches to audio-only.
          </p>
          ...
          <AnimatePresence>
            {bandToast && (
              <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
                className="absolute top-4 left-1/2 -translate-x-1/2 z-20 bg-black/70 text-white text-xs px-4 py-2 rounded-full flex items-center gap-2 whitespace-nowrap"
                role="status" aria-live="polite">
                {band === "critical" ? <WifiOff size={12} className="text-red-400" /> : <Wifi size={12} className="text-amber-400" />}
                {bandToast}
              </motion.div>
            )}
          </AnimatePresence>
          <div className="flex-1 flex items-center justify-center relative">
            {cfg.audioOnly ? (
              <div className="flex flex-col items-center gap-4 text-center">
                ...
                <span className="text-red-400 text-sm">Audio only - low signal</span>
            ...
            <div className={`absolute top-4 left-4 flex items-center gap-1.5 bg-black/50 px-3 py-1.5 rounded-full text-xs ${cfg.textColor}`}>
              {[1,2,3,4].map(i => (
                <div key={i} className={`w-1 rounded-sm ${i <= cfg.bars ? "bg-current" : "bg-white/20"}`} style={{ height: `${i*4+4}px` }} />
              ))}
              {cfg.label}
            </div>
            <div className="absolute top-4 right-4 bg-black/40 text-white text-xs px-3 py-1.5 rounded-full">04:32</div>
          </div>
          {chatOpen && (
            <div className="absolute right-0 top-0 bottom-0 w-72 bg-white flex flex-col border-l border-[#D3D1C7]">
              <div className="p-3 border-b flex items-center justify-between">
                <p className="font-semibold text-sm">Chat</p>
                <button onClick={() => setChatOpen(false)} className="text-xs text-[#5F5E5A]">X</button>
              </div>
              <div className="flex-1 overflow-y-auto p-3 space-y-2" aria-live="polite">
                {messages.map((m, i) => (
                  <div key={i} className={`flex ${m.from === "patient" ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[85%] text-xs px-3 py-2 rounded-2xl ${m.from === "patient" ? "bg-teal-500 text-white" : "bg-gray-100 text-[#2C2C2A]"}`}>{m.text}</div>
                  </div>
                ))}
              </div>
              <div className="p-3 border-t flex gap-2">
                <input value={chatMsg} onChange={e => setChatMsg(e.target.value)} onKeyDown={e => e.key === "Enter" && sendMsg()} placeholder="Type..." className="input-field text-xs py-2 flex-1" />
                <button onClick={sendMsg} className="btn-primary text-xs py-2 px-3">Send</button>
              </div>
            </div>
          )}
          <div className="flex items-center justify-center gap-4 py-5 bg-black/60">
            <button onClick={() => setMicOn(p => !p)} className={`w-12 h-12 rounded-full flex items-center justify-center ${micOn ? "bg-white/20 text-white" : "bg-red-500 text-white"}`}>
              {micOn ? <Mic size={20} /> : <MicOff size={20} />}
            </button>
            <button onClick={() => setCamOn(p => !p)} disabled={cfg.audioOnly} className={`w-12 h-12 rounded-full flex items-center justify-center ${camOn && !cfg.audioOnly ? "bg-white/20 text-white" : "bg-red-500 text-white"}`}>
              {camOn && !cfg.audioOnly ? <Video size={20} /> : <VideoOff size={20} />}
            </button>
            <button onClick={() => setChatOpen(p => !p)} className="w-12 h-12 rounded-full bg-white/20 text-white flex items-center justify-center relative">
              <MessageSquare size={20} />
            </button>
            <button onClick={() => setCallState("ended")} className="w-14 h-14 rounded-full bg-red-500 text-white flex items-center justify-center">
      ...
      {callState === "ended" && (
        <div className="flex-1 flex flex-col items-center justify-center p-6 max-w-md mx-auto w-full space-y-5 animate-fade-in">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-[#5F5E5A] mt-1">Duration: 8 min 22 sec</p>
          </div>
