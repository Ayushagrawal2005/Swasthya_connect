...
export function DoctorHome() {
  ...
  const stats: { labelKey: string; value: number | string; icon: React.ReactNode; subKey: string; subVal: number | null; alert?: boolean }[] = [
    { labelKey: 'patientsToday', value: appointments.length,                                        icon: <User size={16} />,     subKey: 'remaining', subVal: appointments.filter(a => a.status === 'scheduled').length },
    { labelKey: 'avgWaitTime',   value: '8m',                                                        icon: <Clock size={16} />,    subKey: 'estimated', subVal: null },
    ...
    <div className="p-4 sm:p-6 space-y-6 animate-fade-in">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="badge-green">
            <span className="w-1.5 h-1.5 rounded-full bg-green-500 inline-block mr-1" />{t('onDuty')}
          </span>
        </div>
      </div>
      ...
      <section>
        <dl className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {stats.map(s => (
            <div key={s.labelKey} className={`stat-card ${s.alert ? 'border-amber-200 bg-amber-50' : ''}`}>
              <dt className="flex items-center gap-1.5 text-xs text-[#5F5E5A]">
                <span className={s.alert ? 'text-amber-500' : 'text-[#138808]'}>{s.icon}</span>
                {t(s.labelKey as any)}
              </dt>
      ...
      {!loading && pendingReferrals.length > 0 && (
        <div className="flex items-center justify-between bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
          ...
          <button onClick={() => navigate('/doctor/referrals')}
            className="text-xs text-amber-700 font-semibold hover:underline">{t('review')} →</button>
        </div>
      )}
      ...
      <section>
        ...
        {loading ? (
          ...
          <p className="text-sm text-[#5F5E5A] text-center py-8 card">{t('noPatientsInQueue')}</p>
        ) : (
              ...
              <motion.div key={item.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-0.5">
                    ...
                    {item._type === 'referral' ? (
                      <>
                        <span className="badge-amber text-[10px]">{t('referrals')}</span>
                        <span className={`${urgencyBadge[(item as any).urgency] || 'badge-teal'} text-[10px]`}>
      ...
      <div className="flex gap-3 flex-wrap">
        <button onClick={() => navigate('/doctor/patients')}  className="btn-primary text-sm py-2.5 px-4">{t('findPatientBtn')}</button>
        <button onClick={() => navigate('/doctor/referrals')} className="btn-secondary text-sm py-2.5 px-4">{t('referralInboxBtn')}</button>
        <button onClick={() => navigate('/doctor/followup')}  className="btn-secondary text-sm py-2.5 px-4">{t('followUpBoardBtn')}</button>
        <button onClick={() => navigate('/doctor/emergency')} className="btn-coral text-sm py-2.5 px-4">{t('emergencyEscalationBtn')}</button>
      </div>
