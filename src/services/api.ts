...
export const authApi = {
  login: (username: string, password: string) =>
    post<{ token: string; user: { id: string; username: string; role: string; name: string; facilityId?: string; patientId?: string } }>(
...
// ─── Vitals ───────────────────────────────────────────────────────────────────
// Returns the most-recent non-expired vitals for a patient (10-day TTL).
...
export const vitalsApi = {
  getLatest: (patientId: string) =>
    get<LatestVitals>(`/patients/${patientId}/vitals/latest`),
}
...
export const followupsApi = {
  list: (params?: { assignedTo?: string; status?: string; patientId?: string; sourcePortal?: string }) => {
    const p = new URLSearchParams(params as Record<string, string>)
    return get<FollowUp[]>(`/followups?${p}`)
  },
  get: (id: string) => get<FollowUp>(`/followups/${id}`),
  create: (data: {
    patientId: string
    patientName: string
    age?: number
    ...
    sourcePortal?: string
    sourceId?: string
  }) => post<FollowUp>('/followups', data),
  complete: (id: string, notes?: string) => patch<FollowUp>(`/followups/${id}/complete`, { notes }),
  escalate: (id: string, escalatedTo: string, reason: string) => 
    patch<FollowUp>(`/followups/${id}/escalate`, { escalatedTo, reason }),
...
export const notificationsApi = {
  list: (limit?: number) => get<Notification[]>(`/notifications${limit ? `?limit=${limit}` : ''}`),
...
export const wellnessApi = {
  // Family Members
  familyList: () => get<FamilyMember[]>('/wellness/family'),
  familyAdd: (data: Omit<FamilyMember, 'id' | 'guardianPatientId' | 'createdAt'>) =>
    post<FamilyMember>('/wellness/family', data),
  familyDelete: (id: string) => request<{ success: boolean }>('DELETE', `/wellness/family/${id}`),

  // Menstrual Tracker
  menstrualList: () => get<MenstrualCycle[]>('/wellness/menstrual'),
  menstrualAdd: (data: Omit<MenstrualCycle, 'id' | 'patientId' | 'createdAt'>) =>
    post<MenstrualCycle>('/wellness/menstrual', data),
  menstrualDelete: (id: string) => request<{ success: boolean }>('DELETE', `/wellness/menstrual/${id}`),

  // Pregnancy Tracker
  pregnancyList: () => get<PregnancyTracker[]>('/wellness/pregnancy'),
  pregnancyAdd: (data: { lmp: string; edd: string; currentWeek?: number }) =>
    post<PregnancyTracker>('/wellness/pregnancy', data),
  pregnancyUpdate: (id: string, data: Partial<PregnancyTracker>) =>
    patch<PregnancyTracker>(`/wellness/pregnancy/${id}`, data),

  // Vaccinations
  vaccinationsList: () => get<Vaccination[]>('/wellness/vaccinations'),
  vaccinationsAdd: (data: Omit<Vaccination, 'id' | 'createdAt'>) =>
    post<Vaccination>('/wellness/vaccinations', data),
  vaccinationsUpdate: (id: string, data: Partial<Vaccination>) =>
    patch<Vaccination>(`/wellness/vaccinations/${id}`, data),
}
