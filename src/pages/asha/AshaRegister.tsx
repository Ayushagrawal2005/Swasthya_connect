    ...
    return (
      <div className="p-4 sm:p-6 max-w-md mx-auto flex flex-col items-center justify-center min-h-[70vh] space-y-5 text-center animate-fade-in">
        ...
        {form.email && form.password && (
          <div className="card p-5 w-full text-left space-y-3 border-2 border-blue-200 bg-blue-50">
            <div className="flex items-center gap-2 mb-1">
              <User size={15} className="text-blue-600" />
              <p className="text-xs font-semibold text-blue-900">Patient Portal Login Credentials</p>
            </div>
            <div className="space-y-2">
              <div>
                <p className="text-xs text-blue-700 font-medium">Email:</p>
                <p className="text-sm font-mono text-blue-900">{form.email}</p>
              </div>
              <div>
                <p className="text-xs text-blue-700 font-medium">Password:</p>
                <p className="text-sm font-mono text-blue-900">{form.password}</p>
              </div>
            </div>
            <div className="text-xs text-blue-700 bg-white border border-blue-200 rounded-lg px-3 py-2">
              📱 Patient can now log in to the patient portal using these credentials to view health records, track wellness, and manage family health.
            </div>
          </div>
        )}
  ...
  return (
    <div className="p-4 sm:p-6 max-w-xl mx-auto space-y-5 animate-fade-in">
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
        ...
        {/* Email */}
        <div>
          <label htmlFor="reg-email" className="block text-sm font-medium text-[#2C2C2A] mb-1.5">
            Email address <span className="text-red-500">*</span>
          </label>
          <input id="reg-email" type="email" value={form.email}
            onChange={e => set('email', e.target.value)}
            placeholder="patient@example.com" className="input-field"
            aria-required="true" aria-invalid={!!errors.email} />
          <p className="text-xs text-[#5F5E5A] mt-1">For patient portal login</p>
          <AnimatePresence>
            {errors.email && <motion.p role="alert" initial={{opacity:0,y:-4}} animate={{opacity:1,y:0}} exit={{opacity:0}} className="text-xs text-red-600 mt-1">{errors.email}</motion.p>}
          </AnimatePresence>
        </div>

        {/* Password */}
        <div>
          <label htmlFor="reg-password" className="block text-sm font-medium text-[#2C2C2A] mb-1.5">
          ...
          <div className="relative">
            ...
            <AlertCircle className="absolute right-3 top-1/2 -translate-y-1/2 text-[#5F5E5A]" size={16} />
          </div>
          <p className="text-xs text-[#5F5E5A] mt-1">Patient will use this to log into the portal</p>
          <AnimatePresence>
            {errors.password && <motion.p role="alert" initial={{opacity:0,y:-4}} animate={{opacity:1,y:0}} exit={{opacity:0}} className="text-xs text-red-600 mt-1">{errors.password}</motion.p>}
          </AnimatePresence>
        </div>
