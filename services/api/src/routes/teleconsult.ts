...
router.get('/sessions', requireAuth, (req, res) => {
  ...
  sessions = sessions.sort((a, b) => {
    const aPriority = (a as any).priority || 'routine'
    const bPriority = (b as any).priority || 'routine'
    
    const priorityOrder = { emergency: 0, urgent: 1, priority: 2, routine: 3 }
    const aPriorityNum = priorityOrder[aPriority as keyof typeof priorityOrder] || 3
    const bPriorityNum = priorityOrder[bPriority as keyof typeof priorityOrder] || 3
    
    if (aPriorityNum !== bPriorityNum) return aPriorityNum - bPriorityNum
    
    return (a as any).queuePosition - (b as any).queuePosition
  })
  
  res.json(sessions)
})

// GET /teleconsult/queue — prioritized queue for doctors
router.get('/queue', requireAuth, (req, res) => {
  const waiting = db.teleconsultSessions.filter(s => s.status === 'waiting')
  ...
  const enriched = sorted.map(s => {
    ...
    return {
      ...
      triageScore: (s as any).triageScore,
