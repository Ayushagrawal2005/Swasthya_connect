...
import { Users, TrendingUp, AlertTriangle, Package, Activity, ArrowUp, ArrowDown, Clock, Loader2, RefreshCw, ArrowRight } from 'lucide-react'
...
export function AdminOverview() {
  ...
  const kpis = [
    { labelKey: 'totalPatients',    value: (overview?.kpis?.find(k => k.label === 'Total patients')?.value)   ?? '—', changeKey: 'allTime',       up: true,  icon: <Users size={18} />,      color: 'text-[#FF9933]', bg: 'bg-orange-50' },
    { labelKey: 'todaysVisits',     value: (overview?.kpis?.find(k => k.label === "Today's visits")?.value)   ?? '—', changeKey: 'today',         up: true,  icon: <Activity size={18} />,   color: 'text-[#138808]', bg: 'bg-green-50' },
    { labelKey: 'pendingReferrals', value: pendingRefs.length,      changeVal: `${pendingRefs.length}`,      changeKey: 'needReview', up: false, icon: <ArrowRight size={18} />, color: 'text-amber-600', bg: 'bg-amber-50' },
    { labelKey: 'overdueFollowUps', value: overdueFollowups.length, changeVal: `${overdueFollowups.length}`, changeKey: 'overdue',    up: false, icon: <Clock size={18} />,      color: 'text-red-500',   bg: 'bg-red-50' },
    { labelKey: 'stockAlerts',      value: criticalStock.length,    changeKey: 'lowStockItems',              up: false, icon: <Package size={18} />,    color: 'text-amber-600', bg: 'bg-amber-50' },
    { labelKey: 'totalReferrals',   value: referrals.length,        changeKey: 'allTime',                    up: true,  icon: <TrendingUp size={18} />, color: 'text-[#000080]', bg: 'bg-indigo-50' },
  ] as const
          ...
          <div style={{ height: 220 }}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={footfallData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="patients"  name={t('patientsToday')}  stroke="#FF9933" strokeWidth={2} fill="url(#patGrad)" dot={false} />
                <Area type="monotone" dataKey="referrals" name={t('referrals')} stroke="#138808" strokeWidth={2} fill="url(#refGrad)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </section>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <section className="card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => navigate('/admin')} className="text-xs text-[#FF9933] hover:underline">{t('viewAll')}</button>
          </div>
          <div className="space-y-2">
            {loading
              ? <div className="flex justify-center py-4"><Loader2 size={18} className="animate-spin text-[#FF9933]" /></div>
              : pendingRefs.length === 0
                ? <p className="text-xs text-[#5F5E5A] text-center py-4">{t('noPendingReferralsAdmin')}</p>
                : pendingRefs.slice(0, 4).map(r => (
                  <div key={r.id} className="flex items-start gap-2 p-2 rounded-lg bg-amber-50 border border-amber-100">
                    <span className={`${urgencyBadge[r.urgency] || 'badge-teal'} text-[10px] flex-shrink-0 mt-0.5`}>
        ...
        <section className="card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => navigate('/admin/inventory')} className="text-xs text-[#FF9933] hover:underline">{t('manage')}</button>
          </div>
          <div className="space-y-3">
            {loading
              ? <div className="flex justify-center py-4"><Loader2 size={18} className="animate-spin text-[#FF9933]" /></div>
              : criticalStock.length === 0
                ? <p className="text-xs text-[#5F5E5A] text-center py-4">{t('allStockOk')}</p>
        ...
        <section className="card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button className="text-xs text-[#FF9933] hover:underline">{t('fullBoard')}</button>
          </div>
          <div className="space-y-2">
            {loading
              ? <div className="flex justify-center py-4"><Loader2 size={18} className="animate-spin text-[#FF9933]" /></div>
              : overdueFollowups.length === 0
                ? <p className="text-xs text-[#5F5E5A] text-center py-4">{t('noOverdueFollowUps')}</p>
                : overdueFollowups.slice(0, 4).map(f => (
                  <div key={f.id} className="flex items-center gap-2 p-2 rounded-lg bg-red-50 border border-red-100">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-[#5F5E5A] truncate">{f.condition} · {t('due')} {f.dueDate}</p>
                    </div>
                  </div>
                ))
            }
