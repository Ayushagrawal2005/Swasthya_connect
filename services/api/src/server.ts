...
app.post('/auth/login', async (req, res) => {
  try {
    ...
    let user = await db.users.findByUsername(username) as any
    if (!user && username.includes('@')) {
      // If input looks like email, try email lookup
      user = await db.users.findByEmail(username) as any
    }
    ...
    // Include patientId for patient role users
...
app.post('/patients/register', authenticateToken, async (req, res) => {
  try {
    const healthId = `91-${Math.floor(1000 + Math.random() * 9000)}-${Math.floor(1000 + Math.random() * 9000)}-${Math.floor(1000 + Math.random() * 9000)}`
    ...
    // Create user account for patient portal login
    if (req.body.email && req.body.password) {
      try {
        const username = `patient_${healthId.replace(/-/g, '_')}`
        console.log(`📝 Creating user account for patient portal:`, {
          username,
        ...
        const userAccount = await db.users.create({
          username,
          email: req.body.email,
          ...
          patientId: patient.id,
          facilityId: 'patient-portal'
        })
        console.log(`✅ Created patient portal user: ${username} (userId: ${userAccount.id})`);
      } catch (userError: any) {
        console.error('❌ ERROR: Failed to create patient user account:', userError);
...
// Returns the most-recent non-expired vitals recorded for this patient.
// Vitals expire 10 days after the visit was logged (see db.addVisit).
// Response: { vitals, date, expiresAt, visitId } or { vitals: null } if none.
app.get('/patients/:id/vitals/latest', authenticateToken, async (req, res) => {
  try {
    const result = await db.patients.getLatestVitals(req.params.id)
    if (!result) {
      return res.json({ vitals: null, date: null, expiresAt: null, visitId: null })
    }
    res.json(result)
  } catch (error) {
...
// ═══════════════════════════════════════════════════════════════
// WELLNESS & FAMILY HEALTH TRACKING
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
import triageSessionsRoutes from './routes/triageSessions.js'
app.use('/triage-sessions', triageSessionsRoutes)

// ═══════════════════════════════════════════════════════════════
// TELECONSULT QUEUE
// ═══════════════════════════════════════════════════════════════
import teleconsultQueueRoutes from './routes/teleconsultQueue.js'
app.use('/teleconsult-queue', teleconsultQueueRoutes)
