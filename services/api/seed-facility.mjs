...
for (let i = 0; i < 15; i++) {
  ...
  await db.collection(`facilities/${facilityId}/queue`).add({
    ...
    calledAt: i >= 5 ? admin.firestore.Timestamp.now() : null,
    completedAt: i >= 8 ? admin.firestore.Timestamp.now() : null,
...
for (let i = 0; i < 10; i++) {
  ...
  await db.collection(`facilities/${facilityId}/diagnostics`).add({
    ...
    orderedAt: admin.firestore.Timestamp.now(),
    createdAt: admin.firestore.Timestamp.now()
  });
}
...
for (let i = 0; i < 5; i++) {
  ...
  await db.collection('emergencies').add({
    ...
    confirmedAt: i >= 2 ? admin.firestore.Timestamp.now() : null,
    downgraded: false,
    ...
    dispatchedAt: i >= 3 ? admin.firestore.Timestamp.now() : null,
    arrivedAt: null,
    handedOverAt: null,
...
for (let i = 0; i < 8; i++) {
  ...
  await db.collection('referrals').add({
    ...
    acceptedAt: i >= 1 ? admin.firestore.Timestamp.now() : null,
    patientArrivedAt: i >= 3 ? admin.firestore.Timestamp.now() : null,
    consultedAt: i >= 6 ? admin.firestore.Timestamp.now() : null,
    outcome: null,
    stalled: i === 2, // One stalled referral
    stalledAt: i === 2 ? admin.firestore.Timestamp.now() : null,
    ...
    acceptanceDeadline: admin.firestore.Timestamp.fromDate(new Date(Date.now() + 24 * 60 * 60 * 1000)),
    stallThreshold: 48
  });
}
